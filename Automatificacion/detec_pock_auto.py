...
def generate_chimera_script(original_pdb, pockets_pdb, pocket_residues_list, output_script):
    """
    Crea un script para Chimera que:
      - Abre la proteína original (en gris, surface).
      - Abre el PDB con los centroides (predicted pockets) y los muestra como
        esferas amarillas.
      - Abre todos los PDB de residuos, asignando colores distintos a cada uno y
        mostrando su superficie.
    """
    colors = ["red", "green", "blue", "magenta", "cyan", "orange"]

    with open(output_script, 'w') as file:
        # Abrir la proteína original (#0)
        file.write(f"open {original_pdb}\n")
        file.write("surface #0\n")
        file.write("surfrepr mesh #0\n")
        file.write("\n")

        # Abrir el PDB de pockets (modelo #1) y seleccionar resn POC
        file.write(f"open {pockets_pdb}\n")
        # Ajuste importante: usar ':POC' en vez de 'resn POC'
        # para que Chimera reconozca el especificador
        ...
        # Abrir cada PDB de residuos de pocket
        for i, residue_pdb in enumerate(pocket_residues_list, start=2):
            color_to_use = colors[(i - 2) % len(colors)]
            file.write(f"open {residue_pdb}\n")  # modelo #(i)
...
if __name__ == "__main__":
    ...
    # 4. Generar nombres de archivos únicos
    pdb_filename      = get_unique_filename("predicted_pockets", "pdb")  # centroides
    pymol_script      = get_unique_filename("visualize_pockets", "pml")
    chimera_script    = get_unique_filename("visualize_pockets", "cmd")

    # Para almacenar los nombres de los PDB de residuos
    pocket_residues_pdb_list = []

    if len(pocket_clusters) > 0:
        # 4a. Guardar el PDB de centroides
        save_pockets_as_pdb(pocket_clusters, pdb_filename)
        print(f"✅ Pockets file saved as {pdb_filename}")

        # 4b. Para cada pocket, buscar y guardar sus residuos
        for i, pocket in enumerate(pocket_clusters, start=1):
            ...
            pocket_residues_pdb_list.append(pocket_residues_pdb)
            print(f"   Pocket {i} residues saved in: {pocket_residues_pdb}")

        # 4c. Generar scripts de visualización
        pymol_script = get_unique_filename("visualize_pockets", "pml")
        generate_pymol_script(pdb_filename, pymol_script)
        print(f"✅ Visualization script saved as {pymol_script} (Use in PyMOL)")

        chimera_script = get_unique_filename("visualize_pockets", "cmd")
        generate_chimera_script(input_pdb_filename, pdb_filename, 
                                pocket_residues_pdb_list, chimera_script)
