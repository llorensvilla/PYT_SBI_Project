...
def save_pockets_as_pdb(pockets, output_filename):
    ...
    """
    try:
        # Sort the pockets by Surface Area in descending order
        ...
        with open(output_filename, 'w') as file:
            ...
            # Loop through the sorted pockets and write the data
            for i, pocket in enumerate(pockets_sorted):
                ...
                # Formatting the output for better column alignment
                file.write(
...
def generate_pymol_script(original_pdb_path, pockets_pdb_path, pocket_residues_paths, output_script):
    ...
    try:
        with open(output_script, 'w') as file:
            # Optional: black background to mimic the first image style
            file.write("bg_color black\n")
            
            # Load the original protein (absolute path)
            ...
            # Show mesh (wireframe) on top of the protein
            ...
            # Load the predicted pocket centroids (absolute path)
            ...
            # Define a list of colors for pockets/residues
            colors = ["yellow", "red", "green", "blue", "magenta", "cyan", "orange", "purple", "lime", "teal"]
            
            # Color each pocket centroid using the atom id
            ...
            # Load each set of pocket residues as surfaces
            for i, residue_pdb_path in enumerate(pocket_residues_paths):
                ...
                # Show each pocket's residues as a colored surface
...
class BindingSitePredictor:
    ...
    def run(self):
        ...
        # Use absolute paths so PyMOL/Chimera can open from any directory
